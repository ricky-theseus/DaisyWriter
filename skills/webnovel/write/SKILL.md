---
name: webnovel-write
description: 产出可发布章节。单章撰写，三模式：默认/--fast/--minimal。批量模式请用 webnovel-batch。
...
产出可发布章节到 `正文/第{NNNN}章-{title}.md`。默认 2500-3500 字，用户/大纲另有要求时从之。

**前置加载**：写章/审查前必须先 `skill("webnovel-craft")` 加载工艺约束。`webnovel-craft` 定义跨书通用强标准（句长、句式、感官、情绪、五维度），本书 `设定集/行文约束集.md` 只追加本书特殊约束。
...
批量模式（多章自动推进至卷末）请使用 `/webnovel-batch N`。`--stream` 已迁移至 webnovel-batch。

### 模式隔离

- **默认 / `--fast` / `--minimal`**：同步执行。主对话直跑全链路，单章完成即返回。

### 批量模式 → 见 `webnovel-batch` skill

`--stream` 批量架构已独立为 `webnovel-batch` 技能，此处不再维护。功能：主对话启动串行循环，逐章派发独立撰写 agent + 持久审查 agent，后台运行、断点续跑。命令：`/webnovel-batch N`。
...
审查分两阶段：先跑量化扫描脚本（机械），再跑 reviewer agent（定性）。
...
- project_root=${PROJECT_ROOT}
- scripts_dir=${SCRIPTS_DIR}
- prose_report=${量化扫描 JSON 结果}（阶段一产出，内联传入）
- 审查三维度：情节（CBN/CPNs/CEN/禁区） + 设定（OOC/连续性/时间锚点） + **行文品味（暖伤调性、情绪温度、段落节奏感、感官质量）**
- 量化扫描已经覆盖的项（句式计数/词库/句长/静态段落/主动性/情绪单调）不再重复判断——reviewer 专注于 LLM 擅长的品味判断
- **专家审查专项**（对照 `设定集/行文约束集.md` 第七节逐项定性判断）：
  - 信息密度：每500字是否有新信息或剧情推进
  - 主角主动性：是否至少做了一次影响剧情的主动选择
  - 情绪张弛：是否有至少一次压抑→释放的转折
  - 主支线权重：主线篇幅是否大于支线（支线不超40%）
  - 节奏推进：开篇→事件→后果→选择链条是否断裂
...
审查只跑一轮，reviewer 只调用一次。`blocking=true` 的问题在不改剧情、不破设定的前提下定点修复后直接进 Step 4，不重新调用 reviewer；确实无法修复的 blocking 问题用 `AskUserQuestion` 让用户裁决。非 blocking issue 交给 Step 4 处理。`--fast` 只检查 setting/timeline/continuity。

### 审查持久化规则

审查报告（AI产出）是唯一的持久化 artifact。脚本只出数据不存盘。

- **首轮审查**：`审查报告/第{chapter_num}章审查报告.md`
- **修复后重审**：`审查报告/第{chapter_num}章审查报告_二审.md`
- **仍有问题**：`审查报告/第{chapter_num}章审查报告_三审.md`
- 最多三轮。三审仍 blocking → 标记 failed，交用户裁决。
- 每份报告包含：量化扫描摘要 + reviewer 定性判断 + blocking/warning 清单 + 修复建议。
...
python -X utf8 "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" review-pipeline \
  ...
  --review-results "${PROJECT_ROOT}/.webnovel/tmp/review_results.json" \
  --metrics-out "${PROJECT_ROOT}/.webnovel/tmp/review_metrics.json" \
  --report-file "审查报告/第{chapter_num}章审查报告_二审.md" \
  --save-metrics
```
