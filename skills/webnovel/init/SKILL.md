...
在最终确认后、执行生成前，建议运行设定集审核专家组对已收集的设定做质检。

```text
设定已收集完毕，是否由设定审核专家做最终质检？
/webnovel-review-settings --quick
```

审核通过（无阻断问题）→ 进入执行生成。
审核发现阻断问题 → 展示问题清单，用户选择：修改后重审 / 跳过继续 / 中断。
用户选择跳过 → 记录跳过原因到 `.webnovel/logs/run_last.log`。

此步骤不是充分性闸门的硬性条件——用户可跳过。但阻断问题未被修复会导致后续写作反复返工。
